import type { JobPreferences, ProfileFact, StoredJob } from "@campus-job-agent/contracts";
...
export function evaluateJobMatch(job: StoredJob, preferences: JobPreferences, facts: ProfileFact[]): JobMatch {
  ...
  if (job.status === "expired") reasons.push("职位已过期");
  ...
  if (reasons.length > 0) return { job, eligible: false, score: 0, reasons, evidence: [] };
  ...
  const skills = facts.flatMap((fact) => (
    fact.status === "confirmed" && fact.content.type === "skill"
      ? [fact.content.name]
      : []
  ...
  return { job, eligible: true, score: Math.min(score, 100), reasons, evidence };
}
